...
## 2. ¿Por qué el `rendimiento_lag_1` sigue siendo una señal clave en este proyecto?

En la validación actual con ETL v3, el `rendimiento_lag_1` no aparece como un factor "mágico" que siempre domine al 50% en todos los cultivos, pero sí sigue siendo la señal más estable y más difícil de superar en varios escenarios. La evidencia sugiere que la inercia biológica es real, aunque su fuerza depende del cultivo, la ventana temporal y la estrategia de modelado.

Esto se observa en los resultados de validación out-of-time (2019–2024):

- Arroz: Baseline MAE = 0.3120 t/ha vs Híbrido = 0.4800 t/ha
- Cacao: Baseline MAE = 0.0899 t/ha vs Híbrido = 0.0899 t/ha
- Café: Baseline MAE = 0.2319 t/ha vs Híbrido = 0.2047 t/ha (mejora real del híbrido)
- Plátano: Baseline MAE = 1.1595 t/ha vs Híbrido = 1.1595 t/ha

Esto no invalida la intuición biológica del proyecto; la matiza. La interpretación correcta es la siguiente:

1. **La inercia histórica es una señal dominante en cultivos permanentes y de producción sostenida.** El rendimiento anterior explica una parte muy grande del comportamiento del siguiente año, especialmente cuando la variabilidad climática no es extrema.
2. **El ML no siempre supera al baseline por defecto.** En este dataset, el mejor modelo no es universal; depende del cultivo y del nivel de ruido del año de validación.
3. **El valor de la estrategia híbrida no es inexistente.** En Café, el híbrido mejoró de forma consistente respecto al Baseline (~11.8%), lo que confirma que un blend puede aportar valor cuando el historial no es la única pieza relevante. En Arroz, Cacao y Plátano, sin embargo, el Baseline mantuvo igual o mejor desempeño, por lo que la decisión debe tomarse por cultivo y no generalizar una receta única.

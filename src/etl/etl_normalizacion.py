#!/usr/bin/env python3
"""
ETL v3 Mejorado - Normalización SOLO de Features (No de Identificadores)
==========================================================================
Mejora sobre v2: Mantiene columnas de identificación sin normalizar
- anio: Sin cambios (mantiene año original)
- split: Sin cambios (mantiene train/test/historial)
- Normaliza SOLO features numéricas para modelos ML
"""

import csv
import json
from pathlib import Path
from datetime import datetime

class ModelMartETLv3:
    def __init__(self, cultivo):
        self.cultivo = cultivo
        self.archivo_entrada = f"data/processed/model_mart_{cultivo}.csv"
        self.archivo_salida = f"data/processed/etl_normalized_v3/model_mart_{cultivo}.csv"
        self.data = []
        self.headers = []
        self.reporte = {
            "cultivo": cultivo,
            "timestamp": datetime.now().isoformat(),
            "estrategia": "Normalizar features, mantener identificadores",
            "estadisticas_entrada": {},
            "estadisticas_salida": {},
            "transformaciones": []
        }
        self.stats_normalizacion = {}
        self.vars_numericas = []
        self.vars_categoricas = []
        self.columnas_no_normalizar = [
            "departamento", "municipio", "cultivo", "anio", "fuente_eva", "split"
        ]
        
    def cargar(self):
        """Cargar CSV"""
        print(f"\nCargando {self.archivo_entrada}...")
        try:
            with open(self.archivo_entrada, 'r', encoding='utf-8') as f:
                reader = csv.DictReader(f)
                self.headers = reader.fieldnames
                self.data = list(reader)
        except FileNotFoundError:
            print(f"❌ Archivo no encontrado")
            return False
        
        self.reporte["estadisticas_entrada"] = {
            "filas": len(self.data),
            "columnas": len(self.headers)
        }
        
        print(f"✓ {len(self.data)} filas × {len(self.headers)} columnas")
        return True
        
    def identificar_tipos(self):
        """Identificar tipos de datos"""
        self.vars_numericas = []
        self.vars_categoricas = []
        
        for header in self.headers:
            if header in self.columnas_no_normalizar:
                self.vars_categoricas.append(header)
                continue
            
            if not self.data:
                continue
            
            # Buscar primer valor no-vacío
            es_numerico = False
            for fila in self.data:
                v = fila.get(header, "").strip()
                if v and v.lower() not in ["nan", "none"]:
                    try:
                        float(v)
                        es_numerico = True
                        break
                    except ValueError:
                        es_numerico = False
                        break
            
            if es_numerico:
                self.vars_numericas.append(header)
            else:
                self.vars_categoricas.append(header)
        
        print(f"  Variables identificadores (no normalizar): {len(self.columnas_no_normalizar)}")
        print(f"  Variables numéricas (normalizar): {len(self.vars_numericas)}")
        print(f"  Variables categóricas (mantener): {len(self.vars_categoricas) - len(self.columnas_no_normalizar)}")
        
    def convertir_numerico(self, valor):
        """Convertir a número o None"""
        if not valor or valor == "":
            return None
        try:
            v = str(valor).strip()
            if v.lower() in ["nan", "none", ""]:
                return None
            return float(v)
        except ValueError:
            return None
    
    def limpiar(self):
        """Limpieza de datos"""
        print("\nLimpiando datos...")
        
        # Remover duplicados
        filas_antes = len(self.data)
        data_limpia = []
        vistas = set()
        
        for fila in self.data:
            clave = tuple(fila.get(c, "") for c in self.columnas_no_normalizar)
            if clave not in vistas:
                vistas.add(clave)
                data_limpia.append(fila)
        
        self.data = data_limpia
        if filas_antes != len(self.data):
            msg = f"Removidas {filas_antes - len(self.data)} filas duplicadas"
            print(f"  ✓ {msg}")
            self.reporte["transformaciones"].append(msg)
        
        # Imputar NULL en features numéricas
        print("  ✓ Imputando NULL...")
        for col in self.vars_numericas:
            valores = [self.convertir_numerico(fila.get(col)) for fila in self.data]
            valores_validos = [v for v in valores if v is not None]
            
            if not valores_validos:
                continue
            
            # Calcular mediana
            valores_validos.sort()
            if len(valores_validos) % 2 == 0:
                mediana = (valores_validos[len(valores_validos)//2-1] + valores_validos[len(valores_validos)//2]) / 2
            else:
                mediana = valores_validos[len(valores_validos)//2]
            
            # Reemplazar
            for fila in self.data:
                if not fila.get(col) or fila.get(col) == "":
                    fila[col] = str(mediana)
        
        print(f"  ✓ Limpieza completada")
        
    def calcular_estadisticas(self):
        """Calcular media y std para normalización"""
        print("\nCalculando estadísticas...")
        
        for col in self.vars_numericas:
            valores = [self.convertir_numerico(fila.get(col)) for fila in self.data]
            valores_validos = [v for v in valores if v is not None]
            
            if not valores_validos:
                self.stats_normalizacion[col] = {"mean": 0, "std": 1}
                continue
            
            media = sum(valores_validos) / len(valores_validos)
            
            if len(valores_validos) > 1:
                varianza = sum((v - media) ** 2 for v in valores_validos) / len(valores_validos)
                std = varianza ** 0.5
            else:
                std = 1
            
            if std == 0:
                std = 1
            
            self.stats_normalizacion[col] = {"mean": media, "std": std}
        
        print(f"  ✓ {len(self.stats_normalizacion)} variables con parámetros")
        
    def normalizar(self):
        """Normalizar solo features numéricas (no identificadores)"""
        print("\nNormalizando features...")
        
        for fila in self.data:
            for col in self.vars_numericas:
                valor = self.convertir_numerico(fila.get(col))
                if valor is not None:
                    stats = self.stats_normalizacion.get(col, {"mean": 0, "std": 1})
                    valor_normalizado = (valor - stats["mean"]) / stats["std"]
                    fila[col] = str(valor_normalizado)
        
        print(f"  ✓ {len(self.vars_numericas)} features normalizadas")
        
    def guardar(self):
        """Guardar CSV"""
        print(f"\nGuardando...")
        
        Path("data/processed/etl_normalized_v3").mkdir(parents=True, exist_ok=True)
        
        try:
            with open(self.archivo_salida, 'w', newline='', encoding='utf-8') as f:
                writer = csv.DictWriter(f, fieldnames=self.headers)
                writer.writeheader()
                writer.writerows(self.data)
            
            print(f"  ✓ {self.archivo_salida}")
            
            self.reporte["estadisticas_salida"] = {
                "filas": len(self.data),
                "columnas": len(self.headers),
                "columnas_preservadas": len(self.columnas_no_normalizar),
                "features_normalizadas": len(self.vars_numericas)
            }
            
            return True
        except Exception as e:
            print(f"  ❌ Error: {e}")
            return False
        
    def guardar_reporte(self):
        """Guardar JSON"""
        archivo_reporte = f"data/processed/etl_normalized_v3/reporte_{self.cultivo}.json"
        try:
            with open(archivo_reporte, 'w', encoding='utf-8') as f:
                json.dump(self.reporte, f, indent=2, ensure_ascii=False)
            print(f"  ✓ Reporte guardado")
            return True
        except Exception as e:
            print(f"  ❌ Error en reporte: {e}")
            return False
        
    def procesar(self):
        """Pipeline completo"""
        if not self.cargar():
            return None
        
        self.identificar_tipos()
        self.limpiar()
        self.calcular_estadisticas()
        self.normalizar()
        
        if not self.guardar():
            return None
        
        self.guardar_reporte()
        
        print(f"\n✅ {self.cultivo.upper()} completado")
        return self.reporte


def main():
    print("\n" + "="*70)
    print("🔄 ETL v3 - Normalización INTELIGENTE (Identificadores Preservados)")
    print("="*70)
    
    cultivos = ["cafe"]
    reportes = {}
    
    for cultivo in cultivos:
        etl = ModelMartETLv3(cultivo)
        print(f"\n{'─'*70}")
        reporte = etl.procesar()
        if reporte:
            reportes[cultivo] = reporte
    
    # Resumen
    print(f"\n{'='*70}")
    print("✅ RESUMEN FINAL")
    print(f"{'='*70}")
    
    for cultivo, reporte in reportes.items():
        print(f"\n{cultivo.upper()}:")
        print(f"  Entrada:           {reporte['estadisticas_entrada']['filas']} filas")
        print(f"  Salida:            {reporte['estadisticas_salida']['filas']} filas")
        print(f"  Identificadores:   {reporte['estadisticas_salida']['columnas_preservadas']} (sin normalizar)")
        print(f"  Features normalizadas: {reporte['estadisticas_salida']['features_normalizadas']}")
        print(f"  ✅ Listo para entrenar modelos ML")
    
    print(f"\n📁 Archivos generados: data/processed/etl_normalized_v3/")
    print(f"{'='*70}\n")
    
    return True


if __name__ == "__main__":
    success = main()
    exit(0 if success else 1)
